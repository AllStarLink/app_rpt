...
static inline int periodic_process_link(struct rpt *myrpt, struct rpt_link *l, const int elap)
{
	...
	if (newkeytimer_last > 0 && !l->newkeytimer) { /* Translation: We were timing, and it just expired */
		...
		if (l->thisconnected) {
			...
			if (l->link_newkey == RADIO_KEY_NOT_ALLOWED) {
				 ...
				 */
				ast_log(LOG_WARNING,
					"%p newkeytimer expired on connected node, setting newkey from RADIO_KEY_NOT_ALLOWED to RADIO_KEY_ALLOWED.\n", l);
	...
	if (l->outbound) {
		...
		if (!l->chan && !l->retrytimer && l->hasconnected) {
			...
			if (try_reconnect) {
				if (rpt_is_ASLNode(l->name) && (!l->isremote)) {
					l->retries++;
	...
	while (ms >= 0) {
		...
		RPT_LIST_TRAVERSE(myrpt->links, l, l_it) {
			if (l->lastrx) {
				...
				if (l->voterlink)
					myrpt->voteremrx = 1;
				if (rpt_is_ASLNode(l->name)) { /* Ignore '0' nodes */
