...
static inline int process_link_channel(struct rpt *myrpt, struct rpt_link *l, struct ast_channel *who, int *myfirst)
{
	struct rpt_link *m;
	struct ast_frame wf = {
	...
	if (who == l->chan) { /* if it was a read from rx */
		...
		if (f->frametype == AST_FRAME_VOICE) {
			...
			if (l->chan && (l->lastrx || (!altlink(myrpt, l))) &&
				 ...
				 * We may be receiving frames from channel drivers but we discard them and don't pass them on if newkey is set to
				 * != RADIO_KEY_NOT_ALLOWED yet. This happens when the reset code forces it to RADIO_ALLOWED. Of course if
				 * handle_link_data is never called to set newkey to RADIO_KEY_NOT_ALLOWED and stop newkeytimer, then at some
				 * point, we'll set newkey = RADIO_KEY_ALLOWED forcibly (see comments in that part of the code for more info), If
				 * this happens, we're passing voice frames and now sending AST_READIO_KEY messages so we're keyed up and
				 * transmitting, essentially, which we don't want to happen.
				 *
				 */
				ast_write(l->chan, f);
				l->last_frame_sent = 1;
...
static void *rpt(void *this)
{
	...
	while (l != &myrpt->links) {
		...
		ast_softhangup(l->pchan, AST_SOFTHANGUP_DEV);
		l = l->next;
	}
	if (myrpt->xlink == 1)
		myrpt->xlink = 2;
...
static int rpt_handle_call(struct rpt_link *l, struct rpt *myrpt)
{
	int newkeytimer_last, max_retries, elap, myrx, n, ms;
	struct ast_channel *cs[2], *who;
	struct timeval looptimenow, elap_tv_ms, elap_tv_us, residualtime, looptimestart;
	struct rpt_link *m;
	...
	while (!l->killme) {
		...
		if (process_link_channel(myrpt, l, who, &myfirst) == -1) {
			break;
		}
	}
	rpt_mutex_lock(&myrpt->lock);
	...
	ast_hangup(l->pchan);
	rpt_link_free(l);
	return 0; /* l->chan handled by Asterisk cleanup */
}
...
#undef rpt_set_numeric_var_helper
	...
	if (!myrpt->remote) {
		...
		if (!b1[i]) {			/* if not a call-based node number */
			...
			/* if found */
			if (l != &myrpt->links) {
				ast_debug(3, "I found %s in the list, disconnecting it", l->name);
				l->killme = 1;
				l->retries = l->max_retries + 1;
				l->disced = 2;
		...
		return rpt_handle_call(l, myrpt);
	}
