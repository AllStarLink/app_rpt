...
#define AST_AUDIOHOOK_SMALL_QUEUE_TOLERANCE \
	100 /*!< When small queue is enabled, this is the maximum amount of audio that can remain queued at a time. */
...
int ast_audiohook_write_frame(struct ast_audiohook *audiohook, enum ast_audiohook_direction direction, struct ast_frame *frame)
{
	...
	struct ast_slinfactory *other_factory =
		(direction == AST_AUDIOHOOK_DIRECTION_READ ? &audiohook->write_factory : &audiohook->read_factory);
	struct timeval *rwtime = (direction == AST_AUDIOHOOK_DIRECTION_READ ? &audiohook->read_time : &audiohook->write_time),
				   previous_time = *rwtime;
	...
	*rwtime = ast_tvnow();
	...
	if (ast_test_flag(audiohook, AST_AUDIOHOOK_SMALL_QUEUE) &&
		((our_factory_ms > AST_AUDIOHOOK_SMALL_QUEUE_TOLERANCE) || (other_factory_ms > AST_AUDIOHOOK_SMALL_QUEUE_TOLERANCE))) {
	...
	} else if ((ast_test_flag(audiohook, AST_AUDIOHOOK_TRIGGER_MODE) == AST_AUDIOHOOK_TRIGGER_WRITE) &&
	...
	struct ast_audiohook *audiohook, size_t samples, struct ast_frame **read_reference, struct ast_frame **write_reference)
{
	...
	if (!usable_read && !usable_write) {
		...
		ast_debug(
			3, "Read factory %p and write factory %p both fail to provide %zu samples\n", &audiohook->read_factory, &audiohook->write_factory, samples);
		return NULL;
	}
	...
	if (usable_read && !usable_write && (ast_tvdiff_ms(ast_tvnow(), audiohook->write_time) < (samples / 8) * 2) &&
		(ast_slinfactory_available(&audiohook->read_factory) < 2 * samples)) {
	...
	struct ast_frame **write_reference)
{
	...
	if (!ast_test_flag(audiohook, AST_AUDIOHOOK_COMPATIBLE) ||
		(ast_test_flag(audiohook, AST_AUDIOHOOK_COMPATIBLE) && ast_format_get_sample_rate(format) > audiohook->hook_internal_samp_rate)) {
	...
	if (!(read_frame = (direction == AST_AUDIOHOOK_DIRECTION_BOTH ? audiohook_read_frame_both(audiohook, samples, read_reference, write_reference)
...
struct ast_frame *ast_audiohook_read_frame(
	struct ast_audiohook *audiohook, size_t samples, enum ast_audiohook_direction direction, struct ast_format *format)
{
	return audiohook_read_frame_helper(audiohook, samples, direction, format, NULL, NULL);
}

struct ast_frame *ast_audiohook_read_frame_all(
	struct ast_audiohook *audiohook, size_t samples, struct ast_format *format, struct ast_frame **read_frame, struct ast_frame **write_frame)
{
 ...
 */
static struct ast_frame *dtmf_audiohook_write_list(
	struct ast_channel *chan, struct ast_audiohook_list *audiohook_list, enum ast_audiohook_direction direction, struct ast_frame *frame)
{
	struct ast_audiohook *audiohook = NULL;
	int removed = 0;
...
static struct ast_frame *audiohook_list_translate_to_slin(
	struct ast_audiohook_list *audiohook_list, enum ast_audiohook_direction direction, struct ast_frame *frame)
{
	struct ast_audiohook_translate *in_translate =
		(direction == AST_AUDIOHOOK_DIRECTION_READ ? &audiohook_list->in_translate[0] : &audiohook_list->in_translate[1]);
	...
	if (!in_translate->format || ast_format_cmp(frame->subclass.format, in_translate->format) != AST_FORMAT_CMP_EQUAL) {
...
static struct ast_frame *audiohook_list_translate_to_native(
	struct ast_audiohook_list *audiohook_list, enum ast_audiohook_direction direction, struct ast_frame *slin_frame, struct ast_format *outformat)
{
	struct ast_audiohook_translate *out_translate =
		(direction == AST_AUDIOHOOK_DIRECTION_READ ? &audiohook_list->out_translate[0] : &audiohook_list->out_translate[1]);
 ...
 * \retval NULL on failure
 */
static struct ast_frame *audio_audiohook_write_list(
	struct ast_channel *chan, struct ast_audiohook_list *audiohook_list, enum ast_audiohook_direction direction, struct ast_frame *frame)
{
...
{
	return !audiohook_list || (AST_LIST_EMPTY(&audiohook_list->spy_list) && AST_LIST_EMPTY(&audiohook_list->whisper_list) &&
								  AST_LIST_EMPTY(&audiohook_list->manipulate_list));
}

struct ast_frame *ast_audiohook_write_list(
	struct ast_channel *chan, struct ast_audiohook_list *audiohook_list, enum ast_audiohook_direction direction, struct ast_frame *frame)
{
 ...
 * \retval -1 on failure
 */
static int audiohook_volume_callback(
	struct ast_audiohook *audiohook, struct ast_channel *chan, struct ast_frame *frame, enum ast_audiohook_direction direction)
{
...
int ast_audiohook_set_mute_all(struct ast_channel *chan, const char *source, enum ast_audiohook_flags flag, int clearmute)
{
	...
	ast_test_suite_event_notify(
		"AUDIOHOOK_GROUP_MUTE_TOGGLE", "Channel: %s\r\nSource: %s\r\nCount: %d\r\n", ast_channel_name(chan), source, count);
