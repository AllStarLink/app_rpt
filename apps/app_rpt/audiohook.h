 ...
 * \param audiohook
 * \param direction In which direction should the audiohook feed frames, ie if we are snooping 'in', set direction to READ so that
 * only the 'in' frames are fed to the slin factory \retval 0 on success \retval -1 on failure due to audiohook already in use or
 * in shutdown. Can only set direction on NEW audiohooks
 */
...
/*! \brief Reads a frame in from the audiohook structure in mixed audio mode and copies read and write frame data to provided
 * arguments. \param audiohook \param samples Number of samples wanted \param format Format of frame remote side wants back \param
 * read_frame if available, we'll copy the read buffer to this. \param write_frame if available, we'll copy the write buffer to
 * this. \return frame on success \retval NULL on failure
 */
