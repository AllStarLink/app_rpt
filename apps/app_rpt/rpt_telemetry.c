...
static int telem_any(struct rpt *myrpt, struct ast_channel *chan, char *entry, const char *why)
{
	int res;
	char c;
 ...
 */
static int telem_lookup(struct rpt *myrpt, struct ast_channel *chan, char *node, char *name, const char *why)
{
	int res;
	int i;
	char *entry;
	...
	if (entry) {
		if (!ast_strlen_zero(entry) && chan) {
			telem_any(myrpt, chan, entry, why);
		}
	} else {
		res = -1;
	}
...
static void handle_varcmd_tele(struct rpt *myrpt, struct ast_channel *mychannel, char *varcmd)
{
	...
	if (!strcasecmp(strs[0], "COMPLETE")) {
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "COMPLETE");
		if (!res) {
	...
	if (!strcasecmp(strs[0], "PROC")) {
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "patchup", "PROC");
	...
	if (!strcasecmp(strs[0], "TERM")) {
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "patchdown", "TERM");
		if (res < 0) { /* Then default message */
...
void *rpt_tele_thread(void *this)
{
	...
	case REMCOMPLETE:
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "remcomplete", "REMCOMPLETE");
		break;
	case MACRO_NOTFOUND:
		/* wait a little bit */
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) {
...
treataslocal:
		...
		if (haslink) {
			res = telem_lookup(myrpt, mychannel, myrpt->name, "ranger", mytele->mode == UNKEY ? "UNKEY" : "LOCUNKEY");
			if (res)
		...
		if ((mytele->mode == LOCUNKEY) && ((ct = (char *) ast_variable_retrieve(myrpt->cfg, nodename, "localct")))) {	/* Local override ct */
			...
			if (ct_copy) {
				myrpt->noduck = 1;
				res = telem_lookup(myrpt, mychannel, myrpt->name, ct_copy, "LOCUNKEY");
				ast_free(ct_copy);
			} else {
				res = -1;
			}
		...
		if (haslink) {
			myrpt->noduck = 1;
			res = telem_lookup(myrpt, mychannel, myrpt->name, (!hastx) ? "remotemon" : "remotetx", mytele->mode == UNKEY ? "UNKEY" : "LOCUNKEY");
			if (res) {
			...
			if (myrpt->cmdnode[0] && strcmp(myrpt->cmdnode, "aprstt")) {
				...
				res = telem_lookup(myrpt, mychannel, myrpt->name, "cmdmode", mytele->mode == UNKEY ? "UNKEY" : "LOCUNKEY");
				if (res) {
			...
			if (ct_copy) {
				myrpt->noduck = 1;
				res = telem_lookup(myrpt, mychannel, myrpt->name, ct_copy, mytele->mode == UNKEY ? "UNKEY" : "LOCUNKEY");
				ast_free(ct_copy);
			} else {
				res = -1;
			}
	...
	case LINKUNKEY:
		...
		if ((ct = (char *) ast_variable_retrieve(myrpt->cfg, nodename, "linkunkeyct"))) {	/* Unlinked Courtesy Tone */
			ct_copy = ast_strdup(ct);
			if (ct_copy) {
				res = telem_lookup(myrpt, mychannel, myrpt->name, ct_copy, "LINKUNKEY");
				ast_free(ct_copy);
			} else {
				res = -1;
			}
	...
	case SETREMOTE:
		...
		if (!res) {
			if ((!strcmp(myrpt->remoterig, REMOTE_RIG_TM271)) || (!strcmp(myrpt->remoterig, REMOTE_RIG_KENWOOD)))
				telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "SETREMOTE");
			break;
		}
	...
	case REMMODE:
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "REMMODE");
		break;
	case LOGINREQ:
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) {
			break;
	...
	case REMLOGIN:
		...
		res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "REMLOGIN");
		break;
	case REMXXX:
		if (wait_interval(myrpt, DLY_TELEM, mychannel) == -1) {
			break;
		...
		if (strcmp(myrpt->remoterig, REMOTE_RIG_TM271) && strcmp(myrpt->remoterig, REMOTE_RIG_KENWOOD)) {
			...
			if (!res) {
				res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "REMXXX");
			}
		}
		break;
	case SCAN:
	...
	case REMLONGSTATUS:
		...
		if (mytele->mode == REMSHORTSTATUS) {	/* Short status? */
			...
			if (!res) {
				res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", "REMSHORTSTATUS");
			}
			break;
		}
		...
		if (!res) {
			res = telem_lookup(myrpt, mychannel, myrpt->name, "functcomplete", mytele->mode == REMSHORTSTATUS ? "REMSHORTSTATUS" : "REMLONGSTATUS");
		}
		break;
	case STATUS:
		/* wait a little bit */
	...
	case TEST_TONE:
		...
		break;
	case PFXTONE:
		res = telem_lookup(myrpt, mychannel, myrpt->name, "pfxtone", "PFXTONE");
		break;
	default:
		break;
	}
...
void rpt_telemetry(struct rpt *myrpt, int mode, void *data)
{
	...
	case LOCUNKEY:
		...
		if (telem_lookup(myrpt, NULL, myrpt->name, "remotemon", mode == UNKEY ? "UNKEY" : "LOCUNKEY") &&
			telem_lookup(myrpt, NULL, myrpt->name, "remotetx", mode == UNKEY ? "UNKEY" : "LOCUNKEY") &&
			telem_lookup(myrpt, NULL, myrpt->name, "cmdmode", mode == UNKEY ? "UNKEY" : "LOCUNKEY") &&
			(!(v1 && telem_lookup(myrpt, NULL, myrpt->name, v1, mode == UNKEY ? "UNKEY" : "LOCUNKEY"))) &&
			(!(v2 && telem_lookup(myrpt, NULL, myrpt->name, v2, mode == UNKEY ? "UNKEY" : "LOCUNKEY")))) {
			return;
		}
		break;
	case LINKUNKEY:
