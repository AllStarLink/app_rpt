...
#define TELEMETRY_MODES(T) \
	TELEMETRY_MODE(ID) \
	TELEMETRY_MODE(PROC) \
	TELEMETRY_MODE(TERM) \
	TELEMETRY_MODE(COMPLETE) \
	TELEMETRY_MODE(UNKEY) \
	TELEMETRY_MODE(REMDISC) \
	TELEMETRY_MODE(REMALREADY) \
	TELEMETRY_MODE(REMNOTFOUND) \
	TELEMETRY_MODE(REMGO) \
	TELEMETRY_MODE(CONNECTED) \
	TELEMETRY_MODE(CONNFAIL) \
	TELEMETRY_MODE(STATUS) \
	TELEMETRY_MODE(TIMEOUT) \
	TELEMETRY_MODE(ID1) \
	TELEMETRY_MODE(STATS_TIME) \
	TELEMETRY_MODE(PLAYBACK) \
	TELEMETRY_MODE(LOCALPLAY) \
	TELEMETRY_MODE(STATS_VERSION) \
	TELEMETRY_MODE(IDTALKOVER) \
	TELEMETRY_MODE(ARB_ALPHA) \
	TELEMETRY_MODE(TEST_TONE) \
	TELEMETRY_MODE(REV_PATCH) \
	TELEMETRY_MODE(TAILMSG) \
	TELEMETRY_MODE(MACRO_NOTFOUND) \
	TELEMETRY_MODE(MACRO_BUSY) \
	TELEMETRY_MODE(LASTNODEKEY) \
	TELEMETRY_MODE(FULLSTATUS) \
	TELEMETRY_MODE(MEMNOTFOUND) \
	TELEMETRY_MODE(INVFREQ) \
	TELEMETRY_MODE(REMMODE) \
	TELEMETRY_MODE(REMLOGIN) \
	TELEMETRY_MODE(REMXXX) \
	TELEMETRY_MODE(REMSHORTSTATUS) \
	TELEMETRY_MODE(REMLONGSTATUS) \
	TELEMETRY_MODE(LOGINREQ) \
	TELEMETRY_MODE(SCAN) \
	TELEMETRY_MODE(SCANSTAT) \
	TELEMETRY_MODE(TUNE) \
	TELEMETRY_MODE(SETREMOTE) \
	TELEMETRY_MODE(TOPKEY) \
	TELEMETRY_MODE(TIMEOUT_WARNING) \
	TELEMETRY_MODE(ACT_TIMEOUT_WARNING) \
	TELEMETRY_MODE(LINKUNKEY) \
	TELEMETRY_MODE(UNAUTHTX) \
	TELEMETRY_MODE(PARROT) \
	TELEMETRY_MODE(STATS_TIME_LOCAL) \
	TELEMETRY_MODE(VARCMD) \
	TELEMETRY_MODE(LOCUNKEY) \
	TELEMETRY_MODE(METER) \
	TELEMETRY_MODE(USEROUT) \
	TELEMETRY_MODE(PAGE) \
	TELEMETRY_MODE(STATS_GPS) \
	TELEMETRY_MODE(STATS_GPS_LEGACY) \
...
#define rpt_valid_ASLNode(l) ((l[0] > '0') && (l[0] <= '9'))