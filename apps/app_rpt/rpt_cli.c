...
static char *rpt_get_nodes(const char *line, const char *word, int pos, int state, int rpos)
{
	int i = 0;
	int nrpts = rpt_num_rpts();
...
static char *handle_cli_dump(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		return NULL;
	case CLI_GENERATE:
		return rpt_get_nodes(a->line, a->word, a->pos, a->n, 2);
	}
...
static char *handle_cli_setvar(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		return NULL;
	case CLI_GENERATE:
		return rpt_get_nodes(a->line, a->word, a->pos, a->n, 3);
	}
