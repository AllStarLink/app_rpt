...
static int __mklinklist_limit(struct ast_str *buf, int bytes, enum __mklinklist_flags flags)
{
	int new_len, max_len;
	...
	max_len = RPT_AST_STR_MAX_FRAME_SIZE - MAX(sizeof(",000000XX"), sizeof(",X000000"));
	if (new_len > max_len) {
	...
	RPT_LIST_TRAVERSE(myrpt->links, l, l_it) {
		...
		if (flags & USE_FORMAT_RPT_ALINK) {
			...
			*/
			 ...
			 */
			if (__mklinklist_limit(*buf, len + 1, flags)) {
				/* if adding the name, mode, lastrx, and separator will result in fragmentation */
			...
			if (len > 0) {
				 ...
				 */
				while ((len > 0) && __mklinklist_limit(*buf, len, flags)) {
					truncated = 1;
