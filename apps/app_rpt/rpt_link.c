...
int __mklinklist(struct rpt *myrpt,
				 ...
				 int alink_format)
{
	struct rpt_link *l;
	char mode, *links_buf;
	int i, spos, link_count = 0, one_link = 0;
	...
	for (l = myrpt->links.next; l != &myrpt->links; l = l->next) {
		...
		if (alink_format) { /* RPT_ALINK format - only show adjacent nodes*/
			...
			if (ast_str_strlen(l->linklist)) {
				ast_str_append(
				&buf, 0, "%c%s,%s", mode, l->name, ast_str_buffer(l->linklist))
		...
		links_buf = ast_str_buffer(l->linklist);
		for (i = spos; links_buf[i]; i++) {
			if (links_buf[i] == 'T')
				links_buf[i] = mode;
	 ...
	 */
	for (link_count = 0; links_buf[link_count];
...
void rpt_update_links(struct rpt *myrpt)
{
	...
	/* parse em */
	if (n) {
		ast_str_set(&obuf, "%d,%s", n, ast_str_buffer(buf));
	}
	...
	if (n) {
		ast_str_append(&obuf, 0, "%d,%s", n, ast_str_buffer(buf));
	}
