...
static snd_pcm_format_t format = SND_PCCV_FORMAT_S16_LE;
#else
static snd_pcm_format_t format = SND_PCCV_FORMAT_S16_BE;
#endif

static char *config = "beagle.conf";          /* default config file */
#define CONFIG_TUNE_FMT "beagle_tune_%s.conf" /* tune config file */
...
static int beagle_call(struct ast_channel *c, const char *dest, int timeout);
static int beagle_write(struct ast_channel *chan, struct ast_frame *f);
static int beagle_indicate(struct ast_channel *chan, int cond, const void *data,
                           size_t datalen);
static int beagle_fixup(struct ast_channel *oldchan,
                        struct ast_channel *newchan);
static int beagle_setoption(struct ast_channel *chan, int option, void *data,
                            int datalen);
...
static struct ast_channel_tech beagle_tech = {
    .type = "Beagle",
    .description = tdesc,
...
static int16_t hpass6(int16_t input, float *xv, float *yv) {
  ...
  yv[6] = (xv[0] + xv[6]) - 6 * (xv[1] + xv[5]) + 15 * (xv[2] + xv[4]) -
          20 * xv[3] + (-0.3491861578 * yv[0]) + (2.3932556573 * yv[1]) +
...
#define GAIN 1.280673652e+00
  ...
  yv[1] = yv[2];
  yv[2] = yv[3];
  yv[3] = (xv[3] - xv[0]) + 3 * (xv[1] - xv[2]) + (0.5999763543 * yv[0]) +
          (-2.1305919790 * yv[1]) + (2.5161440793 * yv[2]);
  return ((int)yv[3]);
}

static struct chan_beagle_pvt *find_pvt(const char *str) {
  int i;
...
static snd_pcm_t *alsa_card_init(char *dev, snd_pcm_stream_t stream) {
  ...
  if (err < 0) {
    ...
    ast_log(LOG_DEBUG, "Opening device %s in %s mode\n", dev,
            (stream == SND_PCCV_STREAM_CAPTURE) ? "read" : "write");
  ...
  if (period_time > 0)
    err = snd_pcm_hw_params_set_period_time_near(handle, hwparams, &period_time,
                                                 0);
  else
  ...
  if (buffer_time > 0) {
    err = snd_pcm_hw_params_set_buffer_time_near(handle, hwparams, &buffer_time,
                                                 0);
  } else {
  ...
  if (stream == SND_PCCV_STREAM_PLAYBACK)
  ...
  if (stream == SND_PCCV_STREAM_CAPTURE)
    readdev = pfd.fd;
  else
    writedev = pfd.fd;
 ...
 */
static int beagle_digit_begin(struct ast_channel *c, char digit) { return 0; }
...
static int beagle_text(struct ast_channel *c, const char *text) {
  struct chan_beagle_pvt *o = ast_channel_tech_pvt(c);
  char *cmd;
  ...
  if (o->debuglevel)
    ast_verbose(" << Chan_beagle Received beagle text %s >> \n", text);
...
static struct ast_frame *beagle_read(struct ast_channel *c) {
  ...
  if ((chindex == 0) || ((chindex == 1) && (pvts[0].owner == NULL)))
    ismaster = 1;
  ...
  for (;;) {
    ...
    if (n && ((n > 3) || (!o->txkeyed))) {
      ...
      if (n0 && n1)
        n = MIN(f0->datalen, f1->datalen);
      if (n > (FRAME_SIZE * 2))
        n = FRAME_SIZE * 2;
      sp = (short *)(beagle_write_buf);
      sp0 = sp1 = NULL;
      if (f0)
        sp0 = (short *)f0->data.ptr;
      if (f1)
        sp1 = (short *)f1->data.ptr;
      for (i = 0; i < (n / 2); i++) {
        if (f1)
          *sp++ = *sp1++;
        else
  ...
  gv = get_gpios();
  pvts[0].rxhidsq = 0;
  if (gv & MASK_GPIOS_COR0)
    pvts[0].rxhidsq = 1;
  pvts[0].rxhidctcss = 0;
  if (gv & MASK_GPIOS_CTCSS0)
    pvts[0].rxhidctcss = 1;
  pvts[1].rxhidsq = 0;
  if (gv & MASK_GPIOS_COR1)
    pvts[1].rxhidsq = 1;
  pvts[1].rxhidctcss = 0;
  if (gv & MASK_GPIOS_CTCSS1)
    pvts[1].rxhidctcss = 1;
  ast_mutex_lock(&gpio_lock);
  now = ast_tvnow();
  ...
  for (n = 0; n < FRAME_SIZE; n++) {
    if (pvts[1].plfilter && pvts[1].deemphasis)
      *sp1++ =
          hpass6(deemph(*sp++, &pvts[1].destate), pvts[1].hpx, pvts[1].hpy);
    ...
    if (pvts[0].plfilter && pvts[0].deemphasis)
      *sp2++ =
          hpass6(deemph(*sp++, &pvts[0].destate), pvts[0].hpx, pvts[0].hpy);
  ...
  for (m = 0; m < 2; m++) {
    ...
    if (o->lastrx && (!o->rxkeyed)) {
      ...
      if (o->debuglevel)
        ast_verbose("Channel %s RX UNKEY\n", o->name);
    } else if ((!o->lastrx) && (o->rxkeyed)) {
      o->lastrx = 1;
      wf.subclass.integer = AST_CONTROL_RADIO_KEY;
      ast_queue_frame(o->owner, &wf);
      if (o->debuglevel)
        ast_verbose("Channel %s RX KEY\n", o->name);
    }
    ...
    if (o->lasttx && (!n)) {
      ...
      ast_mutex_unlock(&gpio_lock);
      if (o->debuglevel)
        ast_verbose("Channel %s TX UNKEY\n", o->name);
    } else if ((!o->lasttx) && n) {
      o->lasttx = 1;
      ast_mutex_lock(&gpio_lock);
      set_ptt(m, 1);
    ...
    if (((!o->had_gpios_in) || (o->last_gpios_in != gv)) &&
        (!ast_tvzero(o->starttime)) &&
        (ast_tvdiff_ms(ast_tvnow(), o->starttime) >= 550)) {
      char buf1[100];
      struct ast_frame fr;
      ...
      for (i = 1; i <= 5; i++) {
        ...
        if ((!o->had_gpios_in) ||
            ((o->last_gpios_in & gpios_mask[i]) != (gv & (gpios_mask[i])))) {
    ...
    if (ast_channel_state(c) != AST_STATE_UP) {
      continue; /* drop data if frame is not up */
    }
...
static int beagle_fixup(struct ast_channel *oldchan,
                        struct ast_channel *newchan) {
  struct chan_beagle_pvt *o = ast_channel_tech_pvt(newchan);
  o->owner = newchan;
  return 0;
}

static int beagle_indicate(struct ast_channel *c, int cond, const void *data,
                           size_t datalen) {
  struct chan_beagle_pvt *o = ast_channel_tech_pvt(c);
...
static int beagle_setoption(struct ast_channel *chan, int option, void *data,
                            int datalen) {
  char *cp;
  struct chan_beagle_pvt *o = ast_channel_tech_pvt(chan);
  ...
  case AST_OPTION_TONE_VERIFY:
    ...
    case 3:
      ast_log(LOG_DEBUG,
              "Set option TONE VERIFY, mode: DISABLE DETECT(3) on %s\n",
              ast_channel_name(chan));
      o->usedtmf = 0;
      break;
    default:
 ...
 */
static struct ast_channel *
beagle_new(struct chan_beagle_pvt *o, char *ext, char *ctx, int state,
           const struct ast_assigned_ids *assignedids,
           const struct ast_channel *requestor) {
  struct ast_channel *c;

  c = ast_channel_alloc(1, state, o->cid_num, o->cid_name, "", ext, ctx,
                        assignedids, requestor, 0, "beagle/%s", o->name);
  if (c == NULL)
    return NULL;
  ast_channel_tech_pvt_set(c, &beagle_tech);

  if (o == &pvts[0]) {
    if (pvts[1].owner) {
      ast_channel_internal_fd_set(pvts[1].owner, 0, -1);
    }
    ast_channel_internal_fd_set(pvts[0].owner, 0, readdev);
  } else {
    if (!pvts[0].owner) {
      ast_channel_internal_fd_set(pvts[0].owner, 0, readdev);
    }
  ...
  ast_channel_nativeformats_set(c, beagle_tech.capabilities);
  ...
  if (ast_channel_state(c) != AST_STATE_DOWN) {
    if (ast_pbx_start(c)) {
...
beagle_request(const char *type, struct ast_format_cap *cap,
  ...
  if (!(ast_format_cap_iscompatible(cap, beagle_tech.capabilities))) {
  ...
  c = beagle_new(o, NULL, NULL, AST_STATE_DOWN, assignedids, requestor);
  if (c == NULL) {
...
static int beagle_key(int fd, int argc, const char *const *argv) {
...
static int beagle_unkey(int fd, int argc, const char *const *argv) {
  struct chan_beagle_pvt *o = find_pvt(beagle_active);
...
static int beagle_tune(int fd, int argc, const char *const *argv) {
  struct chan_beagle_pvt *o = find_pvt(beagle_active);
  int i = 0;
...
*/
static int beagle_set_debug(int fd, int argc, const char *const *argv) {
  struct chan_beagle_pvt *o = find_pvt(beagle_active);
...
static int beagle_set_debug_off(int fd, int argc, const char *const *argv) {
  struct chan_beagle_pvt *o = find_pvt(beagle_active);
...
static int beagle_active_cmd(int fd, int argc, const char *const *argv) {
  if (argc == 2)
...
static char key_usage[] = "Usage: beagle key\n"
                          "       Simulates COR active.\n";

static char unkey_usage[] = "Usage: beagle unkey\n"
                            "       Simulates COR un-active.\n";

static char active_usage[] =
    "Usage: beagle active [device-name]\n"
    "       If used without a parameter, displays which device is the current\n"
    "one being commanded.  If a device is specified, the commanded radio "
    "device is changed\n"
...
static void store_rxcdtype(struct chan_beagle_pvt *o, const char *s) {
  if (!strcasecmp(s, "no")) {
...
static void store_rxsdtype(struct chan_beagle_pvt *o, const char *s) {
...
static void tune_write(struct chan_beagle_pvt *o) {
  ...
  snprintf(fname, sizeof(fname) - 1, "/etc/asterisk/beagle_tune_%s.conf",
           o->name);
  fp = fopen(fname, "w");
...
static int mixer_write(void) {
  ...
  if (setamixer(0, "AVADC Clock Priority", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Capture Volume", 0, 0) == -1)
    return -1;
  if (setamixer(0, "Analog Left AUXL Capture Switch", 1, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Left Carkit Mic Capture Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Left Headset Mic Capture Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Left Main Mic Capture Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Right AUXR Capture Switch", 1, -1) == -1)
    return -1;
  if (setamixer(0, "Analog Right Sub Mic Capture Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Carkit Playback Volume", 0, 0) == -1)
    return -1;
  if (setamixer(0, "CarkitL Mixer AudioL1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "CarkitL Mixer AudioL2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "CarkitL Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "CarkitR Mixer AudioR1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "CarkitR Mixer AudioR2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "CarkitR Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "DAC Voice Analog Downlink Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "DAC Voice Analog Downlink Volume", 18, -1) == -1)
    return -1;
  if (setamixer(0, "DAC Voice Digital Downlink Volume", 0, -1) == -1)
    return -1;
  if (setamixer(0, "DAC1 Analog Playback Switch", 0, 0) == -1)
    return -1;
  if (setamixer(0, "DAC1 Analog Playback Volume", 18, 18) == -1)
    return -1;
  if (setamixer(0, "DAC1 Digital Coarse Playback Volume", 0, 0) == -1)
    return -1;
  if (setamixer(0, "DAC1 Digital Fine Playback Volume", 50, 50) == -1)
    return -1;
  if (setamixer(0, "DAC2 Analog Playback Switch", 1, 1) == -1)
    return -1;
  if (setamixer(0, "DAC2 Analog Playback Volume", 18, 18) == -1)
    return -1;
  if (setamixer(0, "DAC2 Digital Coarse Playback Volume", 0, 0) == -1)
    return -1;
  v1 = pvts[0].txmixerset * PLAYBACK_MAX / 1000;
  v2 = pvts[1].txmixerset * PLAYBACK_MAX / 1000;
  if (setamixer(0, "DAC2 Digital Fine Playback Volume", v2, v1) == -1)
    return -1;
  if (setamixer(0, "Digimic LR Swap", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Earpiece Mixer AudioL1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Earpiece Mixer AudioL2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Earpiece Mixer AudioR1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Earpiece Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Earpiece Playback Volume", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HS ramp delay", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HandsfreeL Mux", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HandsfreeL Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HandsfreeR Mux", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HandsfreeR Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Headset Playback Volume", 3, 3) == -1)
    return -1;
  if (setamixer(0, "HeadsetL Mixer AudioL1", 1, -1) == -1)
    return -1;
  if (setamixer(0, "HeadsetL Mixer AudioL2", 1, -1) == -1)
    return -1;
  if (setamixer(0, "HeadsetL Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "HeadsetR Mixer AudioR1", 1, -1) == -1)
    return -1;
  if (setamixer(0, "HeadsetR Mixer AudioR2", 1, -1) == -1)
    return -1;
  if (setamixer(0, "HeadsetR Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Left Digital Loopback Volume", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Left1 Analog Loopback Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Left2 Analog Loopback Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PreDriv Playback Volume", 0, 0) == -1)
    return -1;
  if (setamixer(0, "PredriveL Mixer AudioL1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveL Mixer AudioL2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveL Mixer AudioR2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveL Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveR Mixer AudioL2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveR Mixer AudioR1", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveR Mixer AudioR2", 0, -1) == -1)
    return -1;
  if (setamixer(0, "PredriveR Mixer Voice", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Right Digital Loopback Volume", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Right1 Analog Loopback Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Right2 Analog Loopback Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "TX1 Capture Route", 0, -1) == -1)
    return -1;
  ...
  f = 1000.0 / 32.0;
  pvts[0].rxvoiceadj =
      1.0 + (modff(((float)pvts[0].rxmixerset) / f, &f1) * .093981);
  pvts[1].rxvoiceadj =
      1.0 + (modff(((float)pvts[1].rxmixerset) / f, &f1) * .093981);
  if (setamixer(0, "TX1 Digital Capture Volume", v2, v1) == -1)
    return -1;
  if (setamixer(0, "TX2 Capture Route", 0, -1) == -1)
    return -1;
  if (setamixer(0, "TX2 Digital Capture Volume", 0, 0) == -1)
    return -1;
  if (setamixer(0, "Vibra H-bridge direction", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Vibra H-bridge mode", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Vibra Mux", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Vibra Route", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Voice Analog Loopback Switch", 0, -1) == -1)
    return -1;
  if (setamixer(0, "Voice Digital Loopback Volume", 0, -1) == -1)
    return -1;
...
static struct chan_beagle_pvt *store_config(struct ast_config *cfg, char *ctg) {
  ...
  for (v = ast_variable_browse(cfg, ctg); v; v = v->next) {
    ...
    CV_F("ctcssfrom", store_rxsdtype(o, v->value))
  ...
  snprintf(fname, sizeof(fname) - 1, CONFIG_TUNE_FMT, o->name);
  ...
  if (cfg1) {
    for (v = ast_variable_browse(cfg1, o->name); v; v = v->next) {
      ...
      CV_UINT("txmixerset", o->txmixerset)
      CV_END;
    }
...
static char *handle_beagle_key(struct ast_cli_entry *e, int cmd,
...
static char *handle_beagle_unkey(struct ast_cli_entry *e, int cmd,
                                 struct ast_cli_args *a) {
  switch (cmd) {
  case CLI_INIT:
...
static char *handle_beagle_tune(struct ast_cli_entry *e, int cmd,
                                struct ast_cli_args *a) {
  switch (cmd) {
  case CLI_INIT:
...
static char *handle_beagle_debug(struct ast_cli_entry *e, int cmd,
                                 struct ast_cli_args *a) {
  switch (cmd) {
  case CLI_INIT:
...
static char *handle_beagle_debug_off(struct ast_cli_entry *e, int cmd,
                                     struct ast_cli_args *a) {
  switch (cmd) {
  case CLI_INIT:
...
static char *handle_beagle_active(struct ast_cli_entry *e, int cmd,
                                  struct ast_cli_args *a) {
  switch (cmd) {
  case CLI_INIT:
...
static int load_module(void) {
  ...
  ast_log(
      LOG_WARNING,
      "chan_beagle declining module load since WE ARE NOT A BEAGLEBOARD!!\n");
  ...
  gpio = (unsigned long *)mmap(NULL, 0x10000, PROT_READ | PROT_WRITE,
                               MAP_SHARED, hwfd, 0x49050000);
  if (gpio == MAP_FAILED) {
    ast_log(LOG_ERROR, "Can not map memory for GPIO access\n");
    close(hwfd);
    return 0;
  ...
  alsa.icard = alsa_card_init("default", SND_PCCV_STREAM_CAPTURE);
  alsa.ocard = alsa_card_init("default", SND_PCCV_STREAM_PLAYBACK);
  ...
  if (!(beagle_tech.capabilities =
            ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT))) {
    return AST_MODULE_LOAD_DECLINE;
  }
  ast_format_cap_append(beagle_tech.capabilities, ast_format_slin, 0);
  ...
  ast_cli_register_multiple(cli_beagle,
                            sizeof(cli_beagle) / sizeof(struct ast_cli_entry));
...
static int unload_module(void) {
  ...
  ast_cli_unregister_multiple(cli_beagle, sizeof(cli_beagle) /
  ...
  for (i = 0; i < 2; i++) {
    if (pvts[i].dsp)
      ast_dsp_free(pvts[i].dsp);
    if (pvts[i].owner)
      ast_softhangup(pvts[i].owner, AST_SOFTHANGUP_APPUNLOAD);
    /* XXX what about the thread ? */
    /* XXX what about the memory allocated ? */
  }
  ao2_cleanup(beagle_tech.capabilities);
  beagle_tech.capabilities = NULL;
  return 0;
}

AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY,
                         "Beagleboard Radio Interface Channel Driver");
