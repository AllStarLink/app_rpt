...
static char debug_usage[] = "Usage: voter debug level {0-7}\n" "       Enables debug messages in chan_voter\n";
...
static char display_usage[] = "Usage: voter display [instance]\n" "       Display voter instance clients\n";
...
#pragma pack(pop)
	...
	if (debug >= 3)
		ast_verbose("pri thread started on instance %d\n", p->nodenum);
	...
	while (run_forever && (!ast_shutting_down())) {
		...
		if (p->priconn && (ast_tvzero(lasttx) || (voter_tvdiff_ms(tv, lasttx) >= 1000))) {
			...
			if (debug >= 3)
				ast_verbose("Sent pri GPS Keepalive to %s:%d\n", ast_inet_ntoa(p->primary.sin_addr),
		...
		if (i == pri_socket) {
			...
			if (recvlen >= sizeof(VOTER_PACKET_HEADER)) {	/* if set got something worthwile */
				...
				if (debug > 3)
					ast_verbose("Got pri rx packet, len %d payload %d challenge %s digest %08x\n", (int) recvlen,
				...
				if (client->totransmit && (!client->txlockout)) {
					if (IS_CLIENT_PROXY(client)) {
						...
						proxy_audiopacket.vp.curtime.vtime_nsec =
							(client->mix) ? htonl(client->txseqno) : htonl(master_time.vtime_nsec);
						if (debug > 1)
							ast_verbose("sending (proxied) audio packet to client %s digest %08x\n", client->name,
						...
						if (debug > 1)
							ast_verbose("sending audio packet to client %s digest %08x\n", client->name,
										client->respdigest);
		...
		for (client = clients; client; client = client->next) {
			...
			if (voter_tvdiff_ms(tv, client->ping_txtime) >= (PING_TIME_MS * client->pings_sent)) {
				...
				pingpacket.vp.curtime.vtime_nsec =
					(client->mix) ? htonl(client->txseqno) : htonl(master_time.vtime_nsec);
				if (debug > 1)
					ast_verbose("sending ping packet to client %s digest %08x\n", client->name, client->respdigest);
			...
			if (ast_tvzero(client->lastsenttime) || (voter_tvdiff_ms(tv, client->lastsenttime) >= TX_KEEPALIVE_MS)) {
				...
				if (IS_CLIENT_PROXY(client)) {
					...
					proxy_audiopacket.vp.curtime.vtime_nsec =
						(client->mix) ? htonl(client->txseqno) : htonl(master_time.vtime_nsec);
					if (debug > 1)
						ast_verbose("sending (proxied) GPS/Keepalive packet to client %s digest %08x\n", client->name,
									proxy_audiopacket.vp.digest);
					sendto(udp_socket, &proxy_audiopacket, sizeof(VOTER_PACKET_HEADER) + sizeof(VOTER_PROXY_HEADER), 0,
						   (struct sockaddr *) &client->sin, sizeof(client->sin));
				} else {
					if (debug > 1)
						ast_verbose("sending KEEPALIVE (GPS) packet to client %s digest %08x\n", client->name,
									client->respdigest);
										 ...
										 const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	tmp =
		ast_channel_alloc(1, AST_STATE_DOWN, 0, 0, "", (char *) data, context, assignedids, requestor, 0, "voter/%s",
						  (char *) data);
	if (!tmp) {
	...
	while (run_forever && (!ast_shutting_down())) {
		...
		if (i == udp_socket) {	/* if we get a packet */
			...
			if (recvlen >= sizeof(VOTER_PACKET_HEADER)) {	/* if set got something worthwile */
				vph = (VOTER_PACKET_HEADER *) buf;
				if (debug > 3)
					ast_verbose("Got rx packet, len %d payload %d challenge %s digest %08x\n", (int) recvlen,
								ntohs(vph->payload_type), vph->challenge, ntohl(vph->digest));
				client = NULL;
						...
						if (p) {	/* if we found 'em */
							...
							if (client->curmaster) {
								...
								if (ntohs(vph->payload_type) == VOTER_PAYLOAD_PROXY) {
									...
									if (debug > 3)
										ast_verbose
											("Now (proxy) Got rx packet, len %d payload %d challenge %s digest %08x\n",
									...
									if (debug >= 3)
										ast_verbose("Sent outproxy to %s:%d for %s payload %d digest %08x\n",
													ast_inet_ntoa(p->primary.sin_addr), ntohs(p->primary.sin_port),
							...
							if ((index > 0) && (index < (client->buflen - (FRAME_SIZE * 2)))) {
								...
								client->drain40ms = 0;
								if (debug >= 3)
									ast_verbose("mix client %s outa bounds, resetting!!\n", client->name);
							}
							if (client->curmaster) {
								gettimeofday(&tv, NULL);
								for (client = clients; client; client = client->next) {
								...
								for (p = pvts; p; p = p->next) {
									...
									if (maxclient) {
											...
											for (i = 0; i < p->nthresholds; i++) {
												...
												if (p->lastwon->lastrssi >= p->rssi_thresh[i]) {
													...
													if ((i + 1) != p->threshold) {
														...
														p->threshcount = 0;
														if (debug >= 3)
															ast_verbose("New threshold %d, client %s, rssi %d\n",
													...
													/* at the same threshold still, if count is enabled and is met */
														...
														if (debug >= 3)
															ast_verbose
																("Threshold %d time (%d) excedded, client %s, rssi %d\n",
												...
												if (i == (p->nthresholds - 1)) {
													if ((debug >= 3) && p->threshold)
														ast_verbose("Nothing matches criteria any more\n");
													if (p->threshold)
										...
										if (p->lingercount) {
											if (debug >= 3)
												ast_verbose
													("Lingering on client %s, rssi %d, Maxclient is %s, rssi %d\n",
										...
										if (debug > 1)
											ast_verbose("Sending from client %s RSSI %d\n", maxclient->name, maxrssi);
									}
									if ((!p->duplex) && p->txkey) {
										p->rxkey = 0;
										p->lastwon = NULL;
							...
							if (debug > 1)
								ast_verbose("Request for voter client %s to unknown node %d\n",
...
//                          ast_verbose("PING (%s):   seqno: %u  %s.%09d\n",client->name,seqno,timestr,ntohl(vph->curtime.vtime_nsec));
					...
					if (client && client->heardfrom && (ntohs(vph->payload_type) == VOTER_PAYLOAD_GPS) &&
					  ...
					  process_gps:
						...
						if (recvlen == sizeof(VOTER_PACKET_HEADER)) {
							if (debug > 1)
								ast_verbose("Got GPS Keepalive from (%s)\n", client->name);
						} else {
							...
							if (debug > 1)
								ast_verbose("Got GPS (%s): Lat: %s, Lon: %s, Elev: %s\n",
				...
				if (isproxy) {
					if (debug > 1)
						ast_verbose("sending (proxied) packet challenge %s digest %08x password %s\n",
					...
					if (debug > 1)
						ast_verbose("sending packet challenge %s digest %08x password %s\n", authpacket.vp.challenge,
