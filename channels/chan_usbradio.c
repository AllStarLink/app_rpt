...
static struct ast_channel *usbradio_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids,
...
#if 0
	/* error: ‘__builtin_memset’ forming offset [128, 4095] is out of the bounds [0, 128] of object ‘fds’ with type ‘fd_set’ */
	FD_ZERO(&fds);
#else
...
static int usbradio_text(struct ast_channel *c, const char *text)
{
	...
	if (strcmp(cmd, "SETCHAN") == 0) {
		...
		ppbinout(chan);
		if (o->debuglevel)
			ast_log(LOG_NOTICE, "parse usbradio SETCHAN cmd: %s chan: %i\n", text, chan);
		return 0;
	}
	...
	if (strcmp(cmd, "RXCTCSS") == 0) {
		...
		o->rxctcssoverride = !x;
		if (o->debuglevel)
			ast_log(LOG_NOTICE, "parse usbradio RXCTCSS cmd: %s\n", text);
		return 0;
	}
	...
	if (strcmp(cmd, "TXCTCSS") == 0) {
		...
		if (o->debuglevel)
			ast_log(LOG_NOTICE, "parse usbradio TXCTCSS cmd: %s\n", text);
		return 0;
	}
	...
	if (strcmp(cmd, "SETFREQ") == 0) {
		if (o->debuglevel)
			ast_log(LOG_NOTICE, "parse usbradio SETFREQ cmd: %s\n", text);
		tx = strtod(txs, NULL);
		rx = strtod(rxs, NULL);
...
static struct ast_frame *usbradio_read(struct ast_channel *c)
{
	...
	if (o->readerrs)
		ast_log(LOG_WARNING, "Nope, USB read channel [%s] wasn't stuck after all.\n", o->name);
	if (o->devtype != C108_PRODUCT_ID) {
...
#if DEBUG_CAPTURES == 1 && XPMR_DEBUG0 == 1
	if (frxcaptrace && o->b.rxcap2 && o->pmrChan->b.radioactive)
	...
	if (o->pmrChan->rptnum && (o->pmrChan->pLsdCtl->cs[o->pmrChan->rptnum].b.rxkeyed != o->rxlsddecode)) {
		if (o->debuglevel)
			ast_log(LOG_NOTICE, "rxLSDecode = %s, chan %s\n", o->pmrChan->rxctcssfreq, ast_channel_name(o->owner));
	...
	if (o->rxsdtype == SD_HID)
		sd = o->rxhidctcss;
	if (o->rxsdtype == SD_HID_INVERT)
		sd = !o->rxhidctcss;
	if (o->rxsdtype == SD_PP)
		sd = o->rxppctcss;
	if (o->rxsdtype == SD_PP_INVERT)
		sd = !o->rxppctcss;
	...
	if (cd && sd) {
		...
		if (!o->rxkeyed && o->debuglevel)
			ast_log(LOG_NOTICE, "o->rxkeyed = 1, chan %s\n", ast_channel_name(o->owner));
		...
		//if(o->rxkeyed)o->pmrChan->dd.b.doitnow=1;
		if (o->rxkeyed && o->debuglevel)
			ast_log(LOG_NOTICE, "o->rxkeyed = 0, chan %s\n", ast_channel_name(o->owner));
		o->rxkeyed = 0;
		o->rxoncnt = 0;
	}
	...
	if (o->pmrChan->b.txCtcssReady) {
		...
		if (o->debuglevel) {
			ast_log(LOG_NOTICE, "dev=%s got b.txCtcssReady %s\n", o->name, o->pmrChan->txctcssfreq);
		}
	...
	const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor)
{
	...
	c = ast_channel_alloc(1, state, o->cid_num, o->cid_name, "", ext, ctx, assignedids, requestor, 0, "Radio/%s", o->name);
	if (c == NULL)
		return NULL;
...
static int radio_tune(int fd, int argc, const char *const *argv)
{
	...
	if (!strcasecmp(argv[2], "rxnoise"))
		...
		o->b.rxcapraw = o->b.txcapraw = o->b.rxcap2 = o->b.txcap2 = o->pmrChan->b.rxCapture = o->pmrChan->b.txCapture =
			0;
		if (frxcapraw) {
			fclose(frxcapraw);
			frxcapraw = NULL;
...
static char key_usage[] = "Usage: radio key\n" "       Simulates COR active.\n";
...
static void _menu_rxvoice(int fd, struct chan_usbradio_pvt *o, const char *str)
{
	...
	if (o->rxdemod == RX_AUDIO_FLAT) {
		...
		o->rxmixerset = i;
		setamixer(o->devicenum, MIXER_PARAM_MIC_CAPTURE_VOL, o->rxmixerset * o->micmax / 1000, 0);
		setamixer(o->devicenum, MIXER_PARAM_MIC_BOOST, o->rxboostset, 0);
